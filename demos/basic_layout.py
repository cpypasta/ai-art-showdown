...
def Page():
    with solara.Column():
        ...
        with solara.Card("Use solara.Row(justify=space-around) to create a row"):
            with solara.Row(justify="space-around"):
                solara.Info("One the left")
                solara.Info("In the middle")
                solara.Info("On the right")
                
        with solara.Card("Use solara.GridFixed(columns=3)"):
            with solara.GridFixed(columns=3):
                for color in ["green", "red", "orange", "brown", "yellow", "pink"]:
                    solara.Info(color, color=color)
